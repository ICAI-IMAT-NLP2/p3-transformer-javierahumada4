...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = self.cross_attention(hidden_state, enc_output, enc_output, mask=None)
        x = x + hidden_state
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, enc_output: torch.Tensor) -> torch.Tensor:
        ...
        tgt_mask = torch.tril(torch.ones(seq_len, seq_len, device=input_ids.device), diagonal=0)
